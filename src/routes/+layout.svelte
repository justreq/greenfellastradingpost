...
{#if $loadedAuth}
	...
	<main class:pb-32={page.url.pathname != "/" || $currentUser} class="w-screen pt-12 lg:pt-24 min-h-[calc(100vh-9rem)] lg:min-h-[calc(100vh-10rem)] flex flex-col gap-8 sm:gap-16 xl:gap-32">
		{#if dev || ($currentUser && greenfellas.includes($currentUser.id)) || page.route.id?.includes("login") || page.route.id?.includes("signup") || page.route.id?.includes("legal")}
			...
			<section class:hidden={$currentUser || page.url.pathname != "/"} class="w-screen">
				<img alt="Title" id="title" class="mt-56 xl:-mt-20 mb-4 xl:mb-56 w-3/4 sm:w-1/2 lg:w-4/5 lg:max-w-[48rem] mx-auto" draggable="false" />
